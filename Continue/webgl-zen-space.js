...
const ZenSpace = {
  ...
  getRandomMandalaTexture: function() {
    ...
    // Draw mandala
    const mandalas = [
      this.drawFlowerMandala,
      this.drawGeometricMandala,
      this.drawSpirographMandala,
      this.drawSriYantraMandala
    ];
    
    // Select random mandala drawing function
    const drawFunc = mandalas[Math.floor(Math.random() * mandalas.length)];
    drawFunc(ctx, centerX, centerY);
  ...
  // Draw flower mandala on canvas
  drawFlowerMandala: function(ctx, centerX, centerY) {
    // Set styles
    ...
    for (let layer = 0; layer < layers; layer++) {
      ...
      for (let i = 0; i < petalCount; i++) {
        ...
        const petalSize = layerRadius * 0.8;
        
        // Calculate control points for petal curve
        const startX = centerX + Math.cos(angle) * layerRadius * 0.5;
        const startY = centerY + Math.sin(angle) * layerRadius * 0.5;
        
        const endX = centerX + Math.cos(angle + Math.PI / petalCount) * layerRadius * 0.5;
        const endY = centerY + Math.sin(angle + Math.PI / petalCount) * layerRadius * 0.5;
        
        const controlX = centerX + Math.cos(angle + Math.PI / (petalCount * 2)) * petalSize;
        const controlY = centerY + Math.sin(angle + Math.PI / (petalCount * 2)) * petalSize;
  ...
  // Draw geometric mandala on canvas
  drawGeometricMandala: function(ctx, centerX, centerY) {
    // Set styles
    ctx.strokeStyle = 'white';
    ...
    for (let layer = 0; layer < layers; layer++) {
      ...
      if (layer > 0) {
        for (let i = 0; i < sides; i++) {
          const angle = (Math.PI * 2 / sides) * i;
          const innerRadius = radius * layer / layers;
  ...
  // Draw spirograph mandala on canvas
  drawSpirographMandala: function(ctx, centerX, centerY) {
    // Set styles
    ctx.strokeStyle = 'white';
    ctx.lineWidth = 1;
    
    // Parameters
    const R = 100 + Math.random() * 50; // Fixed circle radius
    const r = 30 + Math.random() * 30;  // Moving circle radius
    const d = 50 + Math.random() * 50;  // Distance from center of moving circle
    const loops = 50 + Math.floor(Math.random() * 100);
    
    // Draw spirograph
    ctx.beginPath();
    for (let i = 0; i <= loops; i++) {
      const t = (Math.PI * 2 / loops) * i;
  ...
  // Draw Sri Yantra mandala on canvas
  drawSriYantraMandala: function(ctx, centerX, centerY) {
    // Set styles
    ctx.strokeStyle = 'white';
    ctx.fillStyle = 'rgba(255, 255, 255, 0.05)';
    ...
    // Draw triangles
    const drawTriangle = (size, invert) => {
    ...
    drawTriangle(radius * 0.9, false);
    drawTriangle(radius * 0.8, true);
    drawTriangle(radius * 0.7, false);
    drawTriangle(radius * 0.6, true);
    drawTriangle(radius * 0.5, false);
    drawTriangle(radius * 0.4, true);
    
    // Draw circles
    drawCircle(radius * 0.3);
    drawCircle(radius * 0.2);
    drawCircle(radius * 0.1);
    
    // Draw bindhu (center point)
    ctx.beginPath();
  ...
  updateZenElements: function() {
    ...
    this.zenElements.sacredGeometry.forEach(geometry => {
      ...
      // Floating movement
      const floatY = Math.sin(this.time.elapsed * userData.floatSpeed) * userData.floatAmplitude;
      mesh.position.y += (floatY - (mesh.position.y - mesh.userData.originalY || 0)) * 0.05;
